 ...
 *
 * @method EmpLeaveQuery leftJoinEmpAcc($relationAlias = null) Adds a LEFT JOIN clause to the query using the EmpAcc relation
 * @method EmpLeaveQuery rightJoinEmpAcc($relationAlias = null) Adds a RIGHT JOIN clause to the query using the EmpAcc relation
 * @method EmpLeaveQuery innerJoinEmpAcc($relationAlias = null) Adds a INNER JOIN clause to the query using the EmpAcc relation
 *
...
abstract class BaseEmpLeaveQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `id`, `request`, `status`, `date_started`, `date_ended`, `emp_acc_id`, `list_leave_type_id` FROM `emp_leave` WHERE `id` = :p0';
        try {

...
abstract class BaseEmpAcc extends BaseObject implements Persistent
{
     ...
     */
    protected $collEmpLeaves;
    protected $collEmpLeavesPartial;
    ...
    /**
     * @var        PropelObjectCollection|EmpWork[] Collection to store aggregation of EmpWork objects.
     */
    protected $collEmpWorks;
    protected $collEmpWorksPartial;
     ...
     */
    protected $empLeavesScheduledForDeletion = null;
     ...
     * @var		PropelObjectCollection
     */
    protected $empWorksScheduledForDeletion = null;
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            return $startcol + 8; // 8 = EmpAccPeer::NUM_HYDRATE_COLUMNS.
    ...
    protected function doSave(PropelPDO $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->empLeavesScheduledForDeletion !== null) {
                if (!$this->empLeavesScheduledForDeletion->isEmpty()) {
                    EmpLeaveQuery::create()
                        ->filterByPrimaryKeys($this->empLeavesScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->empLeavesScheduledForDeletion = null;
                }
            }

            if ($this->collEmpLeaves !== null) {
                foreach ($this->collEmpLeaves as $referrerFK) {
            ...
            if ($this->empWorksScheduledForDeletion !== null) {
                if (!$this->empWorksScheduledForDeletion->isEmpty()) {
                    EmpWorkQuery::create()
                        ->filterByPrimaryKeys($this->empWorksScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->empWorksScheduledForDeletion = null;
                }
            }

            if ($this->collEmpWorks !== null) {
                foreach ($this->collEmpWorks as $referrerFK) {
                    if (!$referrerFK->isDeleted() && ($referrerFK->isNew() || $referrerFK->isModified())) {
                        $affectedRows += $referrerFK->save($con);
                    }
                }
    ...
    public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->collEmpLeaves) {
                $result['EmpLeaves'] = $this->collEmpLeaves->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
            ...
            if (null !== $this->collEmpWorks) {
                $result['EmpWorks'] = $this->collEmpWorks->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy && !$this->startCopy) {
            ...
            foreach ($this->getEmpLeaves() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addEmpLeave($relObj->copy($deepCopy));
                }
            }
            ...
            foreach ($this->getEmpWorks() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addEmpWork($relObj->copy($deepCopy));
                }
            }
    ...
    public function initRelation($relationName)
    {
        ...
        if ('EmpWork' == $relationName) {
            $this->initEmpWorks();
        }
    }

    /**
     * Clears out the collEmpLeaves collection
     *
     ...
     * @see        addEmpLeaves()
     */
    public function clearEmpLeaves()
    {
        $this->collEmpLeaves = null; // important to set this to null since that means it is uninitialized
        $this->collEmpLeavesPartial = null;

        return $this;
    }

    /**
     * reset is the collEmpLeaves collection loaded partially
     *
     * @return void
     */
    public function resetPartialEmpLeaves($v = true)
    {
        $this->collEmpLeavesPartial = $v;
    }

    /**
     * Initializes the collEmpLeaves collection.
     *
     * By default this just sets the collEmpLeaves collection to an empty array (like clearcollEmpLeaves());
     ...
     * @return void
     */
    public function initEmpLeaves($overrideExisting = true)
    {
        if (null !== $this->collEmpLeaves && !$overrideExisting) {
            return;
        }
        $this->collEmpLeaves = new PropelObjectCollection();
        $this->collEmpLeaves->setModel('EmpLeave');
    }
     ...
     * @throws PropelException
     */
    public function getEmpLeaves($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collEmpLeavesPartial && !$this->isNew();
        if (null === $this->collEmpLeaves || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collEmpLeaves) {
                // return empty collection
                $this->initEmpLeaves();
            } else {
                $collEmpLeaves = EmpLeaveQuery::create(null, $criteria)
                    ->filterByEmpAcc($this)
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collEmpLeavesPartial && count($collEmpLeaves)) {
                      $this->initEmpLeaves(false);

                      foreach ($collEmpLeaves as $obj) {
                        if (false == $this->collEmpLeaves->contains($obj)) {
                          $this->collEmpLeaves->append($obj);
                        }
                      }

                      $this->collEmpLeavesPartial = true;
                    }

                    $collEmpLeaves->getInternalIterator()->rewind();

                    return $collEmpLeaves;
                }

                if ($partial && $this->collEmpLeaves) {
                    foreach ($this->collEmpLeaves as $obj) {
                        if ($obj->isNew()) {
                            $collEmpLeaves[] = $obj;
                        }
                    }
                ...
                $this->collEmpLeaves = $collEmpLeaves;
                $this->collEmpLeavesPartial = false;
            }
        }

        return $this->collEmpLeaves;
    }

    /**
     * Sets a collection of EmpLeave objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param PropelCollection $empLeaves A Propel collection.
     * @param PropelPDO $con Optional connection object
     * @return EmpAcc The current object (for fluent API support)
     */
    public function setEmpLeaves(PropelCollection $empLeaves, PropelPDO $con = null)
    {
        $empLeavesToDelete = $this->getEmpLeaves(new Criteria(), $con)->diff($empLeaves);


        $this->empLeavesScheduledForDeletion = $empLeavesToDelete;

        foreach ($empLeavesToDelete as $empLeaveRemoved) {
            $empLeaveRemoved->setEmpAcc(null);
        }

        $this->collEmpLeaves = null;
        foreach ($empLeaves as $empLeave) {
            $this->addEmpLeave($empLeave);
        }

        $this->collEmpLeaves = $empLeaves;
        $this->collEmpLeavesPartial = false;
     ...
     * @throws PropelException
     */
    public function countEmpLeaves(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        $partial = $this->collEmpLeavesPartial && !$this->isNew();
        if (null === $this->collEmpLeaves || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collEmpLeaves) {
            ...
            if ($partial && !$criteria) {
                return count($this->getEmpLeaves());
            }
        ...
        return count($this->collEmpLeaves);
    }
     ...
     * @return EmpAcc The current object (for fluent API support)
     */
    public function addEmpLeave(EmpLeave $l)
    {
        if ($this->collEmpLeaves === null) {
            $this->initEmpLeaves();
            $this->collEmpLeavesPartial = true;
        ...
        if (!in_array($l, $this->collEmpLeaves->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddEmpLeave($l);

            if ($this->empLeavesScheduledForDeletion and $this->empLeavesScheduledForDeletion->contains($l)) {
                $this->empLeavesScheduledForDeletion->remove($this->empLeavesScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param	EmpLeave $empLeave The empLeave object to add.
     */
    protected function doAddEmpLeave($empLeave)
    {
        $this->collEmpLeaves[]= $empLeave;
        $empLeave->setEmpAcc($this);
    }

    /**
     * @param	EmpLeave $empLeave The empLeave object to remove.
     * @return EmpAcc The current object (for fluent API support)
     */
    public function removeEmpLeave($empLeave)
    {
        if ($this->getEmpLeaves()->contains($empLeave)) {
            $this->collEmpLeaves->remove($this->collEmpLeaves->search($empLeave));
            if (null === $this->empLeavesScheduledForDeletion) {
                $this->empLeavesScheduledForDeletion = clone $this->collEmpLeaves;
                $this->empLeavesScheduledForDeletion->clear();
            }
            $this->empLeavesScheduledForDeletion[]= clone $empLeave;
            $empLeave->setEmpAcc(null);
        }
     ...
     * been saved, it will retrieve related EmpLeaves from storage.
     *
     ...
     * @return PropelObjectCollection|EmpLeave[] List of EmpLeave objects
     */
    public function getEmpLeavesJoinListLeaveType($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
    {
        $query = EmpLeaveQuery::create(null, $criteria);
        $query->joinWith('ListLeaveType', $join_behavior);

        return $this->getEmpLeaves($query, $con);
    }
    ...
    /**
     * Clears out the collEmpWorks collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     ...
     * @see        addEmpWorks()
     */
    public function clearEmpWorks()
    {
        $this->collEmpWorks = null; // important to set this to null since that means it is uninitialized
        $this->collEmpWorksPartial = null;

        return $this;
    }

    /**
     * reset is the collEmpWorks collection loaded partially
     *
     * @return void
     */
    public function resetPartialEmpWorks($v = true)
    {
        $this->collEmpWorksPartial = $v;
    }

    /**
     * Initializes the collEmpWorks collection.
     *
     * By default this just sets the collEmpWorks collection to an empty array (like clearcollEmpWorks());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @param boolean $overrideExisting If set to true, the method call initializes
     ...
     * @return void
     */
    public function initEmpWorks($overrideExisting = true)
    {
        if (null !== $this->collEmpWorks && !$overrideExisting) {
            return;
        }
        $this->collEmpWorks = new PropelObjectCollection();
        $this->collEmpWorks->setModel('EmpWork');
    }

    /**
     * Gets an array of EmpWork objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
     * @return PropelObjectCollection|EmpWork[] List of EmpWork objects
     * @throws PropelException
     */
    public function getEmpWorks($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collEmpWorksPartial && !$this->isNew();
        if (null === $this->collEmpWorks || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collEmpWorks) {
                // return empty collection
                $this->initEmpWorks();
            } else {
                $collEmpWorks = EmpWorkQuery::create(null, $criteria)
                    ->filterByEmpAcc($this)
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collEmpWorksPartial && count($collEmpWorks)) {
                      $this->initEmpWorks(false);

                      foreach ($collEmpWorks as $obj) {
                        if (false == $this->collEmpWorks->contains($obj)) {
                          $this->collEmpWorks->append($obj);
                        }
                      }

                      $this->collEmpWorksPartial = true;
                    }

                    $collEmpWorks->getInternalIterator()->rewind();

                    return $collEmpWorks;
                }

                if ($partial && $this->collEmpWorks) {
                    foreach ($this->collEmpWorks as $obj) {
                        if ($obj->isNew()) {
                            $collEmpWorks[] = $obj;
                        }
                    }
                ...
                $this->collEmpWorks = $collEmpWorks;
                $this->collEmpWorksPartial = false;
            }
        }

        return $this->collEmpWorks;
    }

    /**
     * Sets a collection of EmpWork objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param PropelCollection $empWorks A Propel collection.
     * @param PropelPDO $con Optional connection object
     * @return EmpAcc The current object (for fluent API support)
     */
    public function setEmpWorks(PropelCollection $empWorks, PropelPDO $con = null)
    {
        $empWorksToDelete = $this->getEmpWorks(new Criteria(), $con)->diff($empWorks);


        $this->empWorksScheduledForDeletion = $empWorksToDelete;

        foreach ($empWorksToDelete as $empWorkRemoved) {
            $empWorkRemoved->setEmpAcc(null);
        }

        $this->collEmpWorks = null;
        foreach ($empWorks as $empWork) {
            $this->addEmpWork($empWork);
        }

        $this->collEmpWorks = $empWorks;
        $this->collEmpWorksPartial = false;
    ...
    /**
     * Returns the number of related EmpWork objects.
     *
     * @param Criteria $criteria
     * @param boolean $distinct
     * @param PropelPDO $con
     * @return int             Count of related EmpWork objects.
     * @throws PropelException
     */
    public function countEmpWorks(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        $partial = $this->collEmpWorksPartial && !$this->isNew();
        if (null === $this->collEmpWorks || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collEmpWorks) {
            ...
            if ($partial && !$criteria) {
                return count($this->getEmpWorks());
            }
            $query = EmpWorkQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collEmpWorks);
    }

    /**
     * Method called to associate a EmpWork object to this object
     * through the EmpWork foreign key attribute.
     *
     * @param    EmpWork $l EmpWork
     * @return EmpAcc The current object (for fluent API support)
     */
    public function addEmpWork(EmpWork $l)
    {
        if ($this->collEmpWorks === null) {
            $this->initEmpWorks();
            $this->collEmpWorksPartial = true;
        ...
        if (!in_array($l, $this->collEmpWorks->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddEmpWork($l);

            if ($this->empWorksScheduledForDeletion and $this->empWorksScheduledForDeletion->contains($l)) {
                $this->empWorksScheduledForDeletion->remove($this->empWorksScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param	EmpWork $empWork The empWork object to add.
     */
    protected function doAddEmpWork($empWork)
    {
        $this->collEmpWorks[]= $empWork;
        $empWork->setEmpAcc($this);
    }

    /**
     * @param	EmpWork $empWork The empWork object to remove.
     * @return EmpAcc The current object (for fluent API support)
     */
    public function removeEmpWork($empWork)
    {
        if ($this->getEmpWorks()->contains($empWork)) {
            $this->collEmpWorks->remove($this->collEmpWorks->search($empWork));
            if (null === $this->empWorksScheduledForDeletion) {
                $this->empWorksScheduledForDeletion = clone $this->collEmpWorks;
                $this->empWorksScheduledForDeletion->clear();
            }
            $this->empWorksScheduledForDeletion[]= clone $empWork;
            $empWork->setEmpAcc(null);
        }
     ...
     * Otherwise if this EmpAcc is new, it will return
     * an empty collection; or if this EmpAcc has previously
     * been saved, it will retrieve related EmpWorks from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in EmpAcc.
     ...
     * @return PropelObjectCollection|EmpWork[] List of EmpWork objects
     */
    public function getEmpWorksJoinListDept($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
    {
        $query = EmpWorkQuery::create(null, $criteria);
        $query->joinWith('ListDept', $join_behavior);

        return $this->getEmpWorks($query, $con);
    }
     ...
     * @return PropelObjectCollection|EmpWork[] List of EmpWork objects
     */
    public function getEmpWorksJoinListPos($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
    {
        $query = EmpWorkQuery::create(null, $criteria);
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep && !$this->alreadyInClearAllReferencesDeep) {
            ...
            if ($this->collEmpLeaves) {
                foreach ($this->collEmpLeaves as $o) {
            ...
            if ($this->collEmpWorks) {
                foreach ($this->collEmpWorks as $o) {
                    $o->clearAllReferences($deep);
                }
            }
        ...
        if ($this->collEmpLeaves instanceof PropelCollection) {
            $this->collEmpLeaves->clearIterator();
        }
        $this->collEmpLeaves = null;
        ...
        $this->collEmpTimes = null;
        if ($this->collEmpWorks instanceof PropelCollection) {
            $this->collEmpWorks->clearIterator();
        }
        $this->collEmpWorks = null;
    }
